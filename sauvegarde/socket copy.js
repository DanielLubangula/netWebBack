  ...
  io.on("connection", (socket) => {
    ...
    socket.on("getOnlineUsers", async () => {
      try {
        ...
        socket.emit("onlineUsersList", users.filter(Boolean));
      } catch (err) {
        console.error("Erreur getOnlineUsers:", err);
        socket.emit("onlineUsersList", { error: "Erreur serveur" });
      }
    });
