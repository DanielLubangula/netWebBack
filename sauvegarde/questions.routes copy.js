...
router.get('/theme/:name/json', (req, res) => {
  ...
  try {
    const rawBlocks = content.split(/###\s*(QCM|VF|Libre)/).slice(1); // [type, bloc, type, bloc...]
    ...
    for (let i = 0; i < rawBlocks.length; i += 2) {
      ...
      // Mélanger les options si le type est QCM
      const shuffledOptions = type === 'QCM' ? shuffleArray(options) : options;

      // Mettre à jour l'index correct après le mélange
      const shuffledCorrectIndex = type === 'QCM' ? shuffledOptions.indexOf(options[correctIndex]) : correctIndex;
    ...
    res.status(200).json({ theme: themeName, questions: shuffledQuestions });
  } catch (err) {
