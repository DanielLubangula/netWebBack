...
// Fonction pour calculer l'XP gagnée/perdue
async function calculateXP(winnerId, loserId) {
  try {
    ...
    // Calcul XP pour le gagnant
    if (levelDiff <= 0) {
      // L'adversaire est de niveau égal ou supérieur
      xpGained = 100 + (Math.abs(levelDiff) * 50);
    }

    // Calcul XP pour le perdant
    let xpLost = 100;
    if (levelDiff >= 0) {
      // L'adversaire est de niveau égal ou inférieur
      xpLost = 100 + (levelDiff * 50);
    }

    return { xpGained, xpLost };
  } catch (error) {
    console.error("Erreur calcul XP:", error);
    return { xpGained: 100, xpLost: 100 };
  }
}

// Fonction pour mettre à jour les stats des joueurs
async function updatePlayerStats(match) {
  try {
    ...
    const winner = players.find(p => p.userId.toString() === winnerId);
    const loser = players.find(p => p.userId.toString() !== winnerId);

    if (!winner || !loser) return;

    // Calculer l'XP
    const { xpGained, xpLost } = await calculateXP(winnerId, loser.userId);
    ...
    await User.findByIdAndUpdate(winnerId, {
      $inc: { 
        experience: xpGained,
        gamesPlayed: 1,
        totalScore: results[winnerId].score
      },
      $set: {
        nextLevelExp: 1000 // Réinitialiser pour le prochain niveau
      }
    });
    ...
    await User.findByIdAndUpdate(loser.userId, {
      $inc: { 
        experience: -xpLost,
        gamesPlayed: 1,
        totalScore: results[loser.userId].score
      }
    });

    // Vérifier les niveaux
    await checkLevelUp(winnerId);
    await checkLevelUp(loser.userId);

    // Mettre à jour les séries
    await updateStreaks(winnerId, loser.userId);

    // Mettre à jour le winrate
    await updateWinRates([winnerId, loser.userId]);
...
// Vérifier si un joueur monte de niveau
    ...
    if (user.experience >= user.nextLevelExp) {
      ...
      await User.findByIdAndUpdate(userId, {
        $set: {
          ...
          nextLevelExp: 1000 + (newLevel * 200) // Augmenter l'XP nécessaire pour le prochain niveau
        }
      });
    }
  } catch (error) {
...
// Mettre à jour les séries de victoires
async function updateStreaks(winnerId, loserId) {
  try {
    // Mettre à jour la série du gagnant
    const winner = await User.findById(winnerId);
    if (winner) {
      const newStreak = winner.currentStreak + 1;
      await User.findByIdAndUpdate(winnerId, {
        $set: { currentStreak: newStreak },
        $max: { bestStreak: newStreak } // Mettre à jour la meilleure série si nécessaire
      });
    }
    ...
    console.error("Erreur mise à jour séries:", error);
  }
}

// Mettre à jour les winrates
async function updateWinRates(userIds) {
  try {
    ...
    console.error("Erreur mise à jour winrate:", error);
  }
}
...
const initializeSocket = (server) => {
  ...
  io.on("connection", async (socket) => {
    ...
    socket.on("acceptChallenge", async ({ toUserId, challengeData }) => {
      ...
      console.log("Room ID généré:", roomId);
          ...
          if (allAnswered) {
            setTimeout(() => {
              ...
              if (nextIndex < match.questions.length) {
                ...
                match.winner = Object.keys(results).reduce((a, b) => 
