...
const initializeSocket = (server) => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.on(
      "acceptChallenge",
      async ({ toUserId, message, challengeData }) => {
        ...
        if (fromSocketId) {
          ...
          try {
            ...
            for (let i = 0; i < rawBlocks.length; i += 2) {
              ...
              const questionLineIndex = lines.findIndex((l) =>
                l.includes("**Question")
              );
            ...
            console.error(
              "Erreur de parsing ou récupération des utilisateurs :",
              err
            );
            io.to(roomId).emit("matchError", {
    ...
    socket.on(
      "answerQuestion",
      ({ roomId, questionId, answerIndex, timeLeft }) => {
        console.log(
          `Réponse reçue dans la salle ${roomId} : Question ${questionId}, Réponse ${answerIndex}, Temps restant ${timeLeft}`
        );

        // Informer tous les joueurs de la réponse donnée
    ...
    socket.on("abandonChallenge", ({ roomId }) => {
      console.log(
        `Défi abandonné dans la salle ${roomId} par ${socket.userId}`
      );

      // Informer tous les joueurs de la salle que le joueur a abandonné
      io.to(roomId).emit("challengeAbandoned", {
        playerId: socket.userId,
        message: "Un joueur a abandonné le défi.",
      });

      // Attribuer la victoire à l'autre joueur
      ...
      if (otherPlayerId) {
        io.to(roomId).emit("challengeFinished", {
          ...
          message: "Le défi est terminé, l'autre joueur a gagné par abandon.",
        });
      }
    ...
    socket.on("finishChallenge", ({ roomId, results }) => {
      console.log(`Défi terminé dans la salle ${roomId}`, results);

      // Envoyer les résultats reçus aux joueurs
